...
manybusesaway.py can also be imported and used for unrelated purposes; the `get_route_listings` function is especially useful. This project is (as far as I can tell) the only library of its kind available. Please see the [license](LICENSE.txt) for more details.
